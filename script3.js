...
function speak(text){
    ...
    let femaleVoice = voices.find(voice => voice.name.includes("Female") || voice.name.includes("Aditi") || voice.name.includes("Samantha"));
   ...
   voice.style.display="none"
    ...
    if(message.includes("hello")||message.includes("hey")||message.includes("hi")||message.includes("hey ki")||message.includes("aur batao")){
      ...
      speak(time)
        ...
        let finalText="this is what i found on internet regarding" + message.replace("KI","") || message.replace("KI","")
        speak(finalText)
        window.open(`https://www.google.com/search?q=${message.replace("KI","")}`,"_blank")
    }
}